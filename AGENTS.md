...
This repository is a Python CLI for VLM-based OCR and issue-targeted recursive refinement. Keep the core workflow provider-neutral so a page can be processed by compatible OpenAI, Anthropic, or future vision-capable model providers.

## Code Organization

- Keep provider-specific SDK calls inside `llm_providers.py`.
- Keep PDF rendering, page orchestration, output writing, and CLI parsing in `ocr.py`.
...
- Do not commit PDFs, rendered page images, logs, local outputs, databases, credentials, or `.env` files.
